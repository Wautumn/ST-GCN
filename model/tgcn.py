...
class tgcnCell(RNNCell):
    ...
    def __init__(self, num_units, adj, num_nodes, input_size=None,
                 act=tf.nn.tanh, reuse=None, index=None):
    ...
    def _gc(self, inputs, state, output_size, bias=0.0, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            weights = tf.get_variable(
                'weights' + self.index, [input_size, output_size], initializer=tf.contrib.layers.xavier_initializer())
            x = tf.matmul(x, weights)  # (batch_size * self._nodes, output_size)
            biases = tf.get_variable(
                "biases" + self.index, [output_size], initializer=tf.constant_initializer(bias, dtype=tf.float32))
